...
function getApiKey() {
    ...
    // Check query params, POST params, or JSON body first (most reliable across web servers)
    $apiKey = $_GET['api_key'] ?? $_GET['key'] ?? $_POST['api_key'] ?? ($input['api_key'] ?? '');
    ...
    if (empty($apiKey) && function_exists('getallheaders')) {
        ...
        foreach ($headers as $name => $value) {
            ...
            if (strcasecmp($name, 'Authorization') === 0 && strpos($value, 'Bearer ') === 0) {
...
$authData = getApiKey();
$apiKey = $authData['key'];
$input = $authData['input'] ?? json_decode(file_get_contents('php://input'), true);
...
if (!$isAuthenticated) {
    ...
    $logMessage = "[" . date('Y-m-d H:i:s') . "] Failed auth attempt from IP: $ipAddress (Key received: '$apiKey')\n";
